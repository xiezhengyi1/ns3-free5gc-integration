...
class GraphMapperTest(unittest.TestCase):
    def test_graph_bundle_contains_flow_slice_and_app_nodes(self) -> None:
        ...
        payload["flows"][0]["telemetry"] = {
            ...
            "packet_sent": 100,
            "packet_received": 99,
            "tick_delta_packet_sent": 11,
            "tick_delta_packet_received": 10,
            "ran": {"ul": {"tx_pkts": 50, "rx_pkts": 45}},
        }
        payload["gnbs"][0]["telemetry"] = {
            "ran": {"active_ue_count": 2, "ul_prb_utilization": 0.6},
            "allocated_dl_mbps": 20.0,
            "allocated_ul_mbps": 10.0,
        }
        ...
        self.assertNotIn(
            "tick_delta_packet_sent",
            summary_nodes["flow:imsi-208930000000001:app-1:flow-1"]["properties"]["telemetry"],
        )
        self.assertNotIn(
            "ran",
            summary_nodes["flow:imsi-208930000000001:app-1:flow-1"]["properties"]["telemetry"],
    ...
    def test_graph_bundle_derives_slice_load_from_utilization_not_allocated_capacity(self) -> None:
        payload = build_payload()
        payload["slices"][0]["resource"] = {
            "capacity_dl_mbps": 100.0,
            "capacity_ul_mbps": 40.0,
        }
        payload["slices"][0]["telemetry"] = {
            "utilization_dl": 0.25,
            "utilization_ul": 0.5,
            "allocated_dl_mbps": 90.0,
            "allocated_ul_mbps": 35.0,
            "demand_dl_mbps": 30.0,
            "demand_ul_mbps": 18.0,
        }
        ...
        slice_props = summary_nodes["slice:01010203"]["properties"]
        self.assertEqual(slice_props["load"]["current_bandwidth_dl"], 25.0)
        self.assertEqual(slice_props["load"]["current_bandwidth_ul"], 20.0)
        self.assertEqual(slice_props["load"]["demand_bandwidth_dl"], 30.0)
        self.assertEqual(slice_props["load"]["demand_bandwidth_ul"], 18.0)
        self.assertEqual(slice_props["telemetry"]["allocated_dl_mbps"], 90.0)
        self.assertEqual(slice_props["telemetry"]["allocated_ul_mbps"], 35.0)

    def test_graph_bundle_prefers_explicit_current_bandwidth_over_utilization(self) -> None:
        payload = build_payload()
        payload["slices"][0]["resource"] = {
            "capacity_dl_mbps": 100.0,
            "capacity_ul_mbps": 40.0,
        ...
        payload["slices"][0]["telemetry"] = {
            "current_bandwidth_dl": 31.0,
            "current_bandwidth_ul": 14.0,
            "utilization_dl": 0.25,
            "utilization_ul": 0.5,
            "allocated_dl_mbps": 90.0,
            "allocated_ul_mbps": 35.0,
        ...
        slice_props = summary_nodes["slice:01010203"]["properties"]
        self.assertEqual(slice_props["load"]["current_bandwidth_dl"], 31.0)
        self.assertEqual(slice_props["load"]["current_bandwidth_ul"], 14.0)
        self.assertEqual(slice_props["telemetry"]["allocated_dl_mbps"], 90.0)
        self.assertEqual(slice_props["telemetry"]["allocated_ul_mbps"], 35.0)
